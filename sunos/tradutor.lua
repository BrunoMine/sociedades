...
-- Tradução intllib
sunos.intllib = {}
sunos.intllib.S, sunos.intllib.NS = dofile(modpath.."/lib/intllib.lua")
...
-- Gera arquivos de tradução sunos.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por sunos apartir de "..file.."\n# textdomain: sunos\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/sunos."..lang_code..".tr", "w")
...
local s = minetest.get_translator("sunos")
sunos.s = function(...)
	local args = { ... }
